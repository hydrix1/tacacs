...
static void catch_test_get_missing_argument(const char* test_name,
                                            const char* prompt, ...)
{
    pid_t child;
    char* file_base = 0;
    ...
    asprintf(&file_base, "get_missing_argument_%s", test_name);
...
void test_entry()
{
    /* Test get_missing_argument() */
    unity_start_group("get_missing_argument");
    catch_test_get_missing_argument("one_word", "one word prompt");
